...
@section('content')
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary btn-sm editBtn"
                                                data-id="{{ $package->id }}" data-amount="{{ $package->package_amount }}"
                                                data-tax="{{ $package->tax }}" data-service="{{ $package->service }}"
                                                data-full="{{ $package->full }}">
                                                Edit
                                            </button>

                                            <form id="delete-form-{{ $package->id }}"
                                                action="{{ route('packages.delete', $package->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                ...
                <form action="{{ route('packages.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" name="package_amount" id="package_amount"
                        ...
                        <div class="mb-3">
                            <label for="service" class="form-label">Service Amount</label>
                            <input type="text" name="service" id="service" class="form-control" required>
                        </div>
                        <div class="mb-3">
                            <label for="full" class="form-label">Full Amount</label>
                            <input type="number" step="0.01" name="full" id="full" class="form-control"
                                required>
                        </div>
                    </div>
                ...
                <form action="{{ route('packages.update') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" name="package_amount" id="update_package_amount"
                                class="form-control" required>
                        </div>
                        <div class="mb-3">
                            <label for="update_tax" class="form-label">Tax</label>
                            <input type="number" step="0.01" name="tax" id="update_tax" class="form-control"
                                required>
                        </div>
                        <div class="mb-3">
                            <label for="update_service" class="form-label">Service Amount</label>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" name="full" id="update_full" class="form-control"
                                required>
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.editBtn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    document.getElementById('update_full').value = this.dataset.full;
        ...
        function calculateFullAmount(addMode = true) {
            ...
            document.getElementById(prefix + 'full').value = (amount + tax + service).toFixed();
        }

        // For Add Form
        ['package_amount', 'tax', 'service'].forEach(id => {
            document.getElementById(id).addEventListener('input', () => calculateFullAmount(true));
        });

        // For Edit Form
        ['update_package_amount', 'update_tax', 'update_service'].forEach(id => {
            document.getElementById(id).addEventListener('input', () => calculateFullAmount(false));
        });
