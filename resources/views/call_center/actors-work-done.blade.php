...
@section('content')
    <div>
        <div class="card shadow-sm mb-3">
            <div class="card-body">
                {{-- FILTER FORM --}}
                <form method="GET" action="{{ route('actor.work.index') }}" class="row g-2 align-items-end">
                  ...
                  <div class="col-auto">
                    ...
                    <select name="work_type" class="form-select form-select-sm">
                      ...
                      @foreach($workTypes as $wt)
                        <option
                          value="{{ $wt->name }}"
                          {{ request('work_type') == $wt->name ? 'selected' : '' }}
                        >{{ $wt->name }}</option>
                      @endforeach
                    </select>
                  </div>
                  ...
                  <div class="col-auto">
                    ...
                    <a href="{{ route('actor.work.index') }}" class="btn btn-outline-secondary btn-sm">
                      Clear
                    </a>
                  </div>
                </form>
                                    ...
                                    @if (Auth::user()->role !== 'act')
                                        <td>
                                            <div class="d-flex gap-1">
                                                ...
                                                <form action="{{ route('actor.work.destroy', $entry->id) }}"
                                                      method="POST">
                                                    @csrf @method('DELETE')
                                            ...
                                            <div class="modal fade" id="editModal{{ $entry->id }}" tabindex="-1"
                                                ...
                                                <div class="modal-dialog">
                                                    <form action="{{ route('actor.work.update', $entry->id) }}" method="POST"
                                                        class="modal-content">
                                                        @csrf
