...
class SlipController extends Controller
{
    public function store(Request $request)
    {
        ...
        // 4) Load invoice & check if it's the first payment
        $invoice = Invoice::where('inv', $request->inv)->firstOrFail();
        $wasAmt1Null = is_null($invoice->amt1); // ✅ check before modifying amt1
        $now = Carbon::now();

        // 5) Completed payment path
        if ($request->payment_type === 'completed') {
            $orderData = ['payment_status' => 'done', 'advance' => 0, 'ps' => '1'];

            if ($wasAmt1Null) {
                $orderData['created_at'] = $now;
            }

            Order::where('invoice', $request->inv)->update($orderData);
        ...
        } elseif (is_null($invoice->amt2)) {
            $invoice->amt2 = $totalAdvance;
        } elseif (is_null($invoice->amt3)) {
